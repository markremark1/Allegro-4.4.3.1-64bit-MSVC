...
*/
int GetChild(struct BoundingBox *b, float qx, float qy)
{
   int ret = 0;
...
*/
float GetX1(int child, struct BoundingBox *b)
{
   return (child & 1) ? CentreX(b) : b->TL.Pos[0];
}
float GetX2(int child, struct BoundingBox *b)
{
   return (child & 1) ? b->BR.Pos[0] : CentreX(b);
}
float GetY1(int child, struct BoundingBox *b)
{
   return (child & 2) ? CentreY(b) : b->TL.Pos[1];
}
float GetY2(int child, struct BoundingBox *b)
{
...
*/
int ToggleChildPtrY(float yvec, int oldchild)
{
...
int ToggleChildPtrX(float xvec, int oldchild)
{
                ...
                int divider)
{
   ...
   if (Tree->Children) {
      ...
      if ((Tree->NumContents == divider)
          && ((Tree->Bounder.BR.Pos[0] - Tree->Bounder.TL.Pos[0]) > SCREEN_W)
          && ((Tree->Bounder.BR.Pos[1] - Tree->Bounder.TL.Pos[1]) > SCREEN_H)) {
...
*/
struct QuadTreeNode *GetNode(struct QuadTreeNode *Ptr, float *pos, float *vec)
{
   int Child;
   int MidX, MidY;
...
struct QuadTreeNode *GetCollisionNode(struct Level *lvl, float *pos,
                                      float *vec)
{
   return GetNode(&lvl->CollisionTree, pos, vec);
}

struct QuadTreeNode *GetObjectNode(struct Level *lvl, float *pos, float *vec)
{
...
*/
void DrawEdge(BITMAP *target, struct Triangle *tri,
   ...
   while (c--) {
      ...
      if (tri->EdgeFlags[c] & TRIFLAGS_EDGE) {
         /*
            ...
            We're going to use quad3d with an affine texuring mode to
            paint our polygon edge. In order to do that with Allegro we must
            fill some V3D structs.

            Texture u is determined according to world position so that
            edges that should appear to meet up do.

          */
         PolyEdges[0].x = PolyEdges[3].x =
            ftofix(tri->Edges[c]->Pos[0] - ScrBounder->TL.Pos[0]);
         PolyEdges[0].u = PolyEdges[3].u = ftofix(tri->Edges[c]->Pos[0]);
         PolyEdges[0].y =
            ftofix(tri->Edges[c]->Pos[1] - ScrBounder->TL.Pos[1]) -
            itofix(tri->EdgeFlags[c] & TRIFLAGS_WIDTH);
         PolyEdges[3].y =
            ftofix(tri->Edges[c]->Pos[1] - ScrBounder->TL.Pos[1]) +
            itofix(tri->EdgeFlags[c] & TRIFLAGS_WIDTH);

         PolyEdges[1].x = PolyEdges[2].x =
            itofix(tri->Edges[c2]->Pos[0] - ScrBounder->TL.Pos[0]) +
            itofix(1);
         PolyEdges[1].u = PolyEdges[2].u = ftofix(tri->Edges[c2]->Pos[0]);
         PolyEdges[2].y =
            ftofix(tri->Edges[c2]->Pos[1] - ScrBounder->TL.Pos[1]) +
            itofix(tri->EdgeFlags[c2] & TRIFLAGS_WIDTH);
         PolyEdges[1].y =
            ftofix(tri->Edges[c2]->Pos[1] - ScrBounder->TL.Pos[1]) -
            itofix(tri->EdgeFlags[c2] & TRIFLAGS_WIDTH);
          ...
          * drawing here. */
         set_alpha_blender();
         quad3d(target, POLYTYPE_ATEX_TRANS, tri->Material->Edge,
...
*/
void DrawTriangle(BITMAP *target, struct Triangle *tri,
                  struct BoundingBox *ScrBounder)
{
   /* quick bounding box check - if the triangle isn't on screen then we can
      forget about it */
   ...
   /*
      drawing is quickly and easily done using a patterned fill of an ordinary
      triangle. Notice that the pattern offset is set according to the offset
      of our view into the world so that it moves correctly with the triangle
    */
   drawing_mode(DRAW_MODE_COPY_PATTERN, tri->Material->Fill,
                -ScrBounder->TL.Pos[0], -ScrBounder->TL.Pos[1]);
   triangle(target, tri->Edges[0]->Pos[0] - ScrBounder->TL.Pos[0],
            tri->Edges[0]->Pos[1] - ScrBounder->TL.Pos[1],
            tri->Edges[1]->Pos[0] - ScrBounder->TL.Pos[0],
...
*/
void DrawObject(BITMAP *target, struct Object *obj,
                struct BoundingBox *ScrBounder)
{
   /* quick bounding box check - if the triangle isn't on screen then we can
      forget about it */
   ...
   if (obj->Flags & OBJFLAGS_VISIBLE)
      rotate_scaled_sprite(target, obj->ObjType->Image,
                           (int)(0.5f *
                                 (obj->Bounder.TL.Pos[0] +
                                  obj->Bounder.BR.Pos[0]) -
                                 ScrBounder->TL.Pos[0]) -
                           (obj->ObjType->Image->w >> 1),
                           (int)(0.5f *
                                 (obj->Bounder.TL.Pos[1] +
                                  obj->Bounder.BR.Pos[1]) -
                                 ScrBounder->TL.Pos[1]) -
                           (obj->ObjType->Image->h >> 1), obj->Angle,
                           ftofix((float)SCREEN_H / 480.0f));
}
...
void DrawQuadTreePart(BITMAP *target, struct Level *Lvl,
                      struct BoundingBox *ScrBounder,
   ...
   while (Visible) {
      ...
      while (Thing && Thing->Type == OBJECT) {
         if (Thing->Content.O->LastFrame != framec) {
            DrawObject(target, Thing->Content.O, ScrBounder);
      ...
      while (Thing) {
         if (Thing->Content.T->LastFrame != framec)
            DrawTriangle(target, Thing->Content.T, ScrBounder);
         Thing = Thing->Next;
      }
      ...
      while (Thing) {
         if (Thing->Content.T->LastFrame != framec) {
            DrawEdge(target, Thing->Content.T, ScrBounder);
...
void BeginQuadTreeDraw(BITMAP *target, struct Level *Lvl,
                       ...
                       struct BoundingBox *ScrBounder, unsigned int framec)
{
   ...
   DrawQuadTreePart(target, Lvl, ScrBounder, framec, FALSE);
}

void EndQuadTreeDraw(BITMAP *target, struct Level *Lvl,
                     struct BoundingBox *ScrBounder, unsigned int framec)
{
   DrawQuadTreePart(target, Lvl, ScrBounder, framec, TRUE);
}
