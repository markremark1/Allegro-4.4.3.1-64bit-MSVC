...
*/
float FixUp(float *vec, float normalx, float normaly)
{
   float mul;
...
*/
void SetAngle(float normalx, float normaly, float *a, int JustDoIt)
{
   float NewAng = atan2(normalx, -normaly);

   if (JustDoIt || ((NewAng < (AL_PI * 0.25f)) && (NewAng >= 0)) ||
       ((NewAng > (-AL_PI * 0.25f)) && (NewAng <= 0)) ||
       fabs(NewAng - *a) < (AL_PI * 0.25f)
      )
      *a = NewAng;
}
...
*/
void DoFriction(float r, struct Edge *E, float *vec)
{
...
{
   float MoveVec[2];
   float CollTime, NCollTime;
   float End[2], ColPoint[2];
   void *CollPtr;
   float d1, d2;
   struct Edge *E;
   float r;
...
struct QuadTreeNode *RunPhysics(struct Level *lvl, float *pos, float *vec, float TimeToGo, struct Animation *PAnim)
{
...
struct QuadTreeNode *RunPhysics(struct Level *lvl, float *pos,
                                float *vec, float TimeToGo,
                                struct Animation *PAnim)
{
   float MoveVec[2];
